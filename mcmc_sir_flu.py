...
"""
Formulación de un modelo dinámico tipo SIR, y un modelo bayesiano para calcular R0 
...
class forward_mapping:
    ...
    def rhs(self,x,t,p):    
        ...
        fx[0] =  -p[0]*x[0]*x[1]
        fx[1] = p[0]*x[0]*x[1] - self.gamma*x[1]
        fx[2] = self.gamma*x[1]  #pronostico de sistema dinamico es el objetivo
        return fx
    ...
    def solve(self,p):
        ...
        result = np.zeros(nn)
        for k in range(nn):
            x_e = soln[10*k:10*(k+1)+1,1]
            x_r = soln[10*k:10*(k+1)+1,0]
            incidence = self.sigma*x_e*x_r*self.N
...
if __name__=="__main__":
    ...
    np.savetxt('chain2.txt',seir.Output)
...
S_chain = seir[1200:, 3]
plt.hist(S_chain)
plt.axvline(x = np.mean(S_chain), color="green")
plt.axvline(x = np.percentile(S_chain, 97.5), color="red")
plt.axvline(x = np.percentile(S_chain, 2.5), color="red")
print(np.mean(S_chain))
print(np.percentile(S_chain, 2.5))
print(np.percentile(S_chain, 97.5))

#ESTIMACIÓN DEL R_0
mu = 1.0/(75.0*365.0)
sigma = 1.0/3.0
gamma = 1.0/5.0
R0 = (seir.Output[1200:, 0]/gamma)
print(np.mean(R0))
plt.hist(R0)
...
def rhs(x,t,p):    
        ...
        inc = p[0]*x[1]     #cuadrar parametros que se postulan (dejarlos quietos) y los que se infieren si molestarlos
        fx[0] =  -p[0]*x[0]*x[1]
        fx[1] = p[0]*x[0]*x[1] - gamma*x[1]
        fx[2] = gamma*x[1]  #pronostico de sistema dinamico es el objetivo
        return fx
